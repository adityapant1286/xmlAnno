...
import com.xmlanno.reflection.scanners.TypeElementsScanner;
...
import static com.xmlanno.reflection.utils.XmlAnnoUtil.prepareFile;
...
public class JavaCodeSerializer implements Serializer {
    ...
    public File save(Reflections reflections, String name) {
        ...
        if (name.endsWith("/")) name = name.substring(0, name.length() - 1); //trim / at the end
        ...
        packageName = lastDot == -1
                        ? ""
                        : name.substring(name.lastIndexOf('/') + 1, lastDot);
        className = lastDot == -1
...
//            Files.write(sb.toString(), new File(filename), Charset.defaultCharset());
            Files.asCharSink(new File(filename), Charset.defaultCharset()).write(sb.toString());
